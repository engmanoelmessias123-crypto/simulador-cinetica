...
with c_t2:
    ...
    st.markdown("1. Use a Velocidade Instantânea acima.\n2. Salve pontos em tempos diferentes.")
    linearizar_dif = st.checkbox("Plotar ln(v) vs ln[C]", key="LOGLOG")
    ...
    if 'pontos_taxa' in st.session_state and st.session_state.pontos_taxa:
        ...
        if linearizar_dif:
            ...
            if mostrar_tendencia:
                if len(set(v_vals)) == 1 or (ordem_a_sid == 0 and not modo_desafio): # Correção para Ordem Zero
                    fig_taxa.add_trace(go.Scatter(x=[c_vals.min(), c_vals.max()], y=[v_vals[0], v_vals[0]], mode='lines', name='Ajuste (v=k)', line=dict(color='yellow', dash='dash')))
                elif len(set(c_vals)) > 1:
                    z = np.polyfit(lc, lv, 1)
                    cs = np.linspace(c_vals.min(), c_vals.max(), 100)
                    fig_taxa.add_trace(go.Scatter(x=cs, y=np.exp(z[1])*(cs**z[0]), mode='lines', name='Ajuste Curva', line=dict(color='yellow', dash='dash')))
...
with c_l2:
    ...
    t_r_max = float(t[-1])
    t1_l = st.number_input("Tempo t1", 0.0, t_r_max, float(t_r_max/4), key="L1")
    t2_l = st.number_input("Tempo t2", 0.0, t_r_max, float(t_r_max/2), key="L2")
