...
with c_t1:
    ...
    if 'pontos_taxa' in st.session_state and len(st.session_state.pontos_taxa) > 0:
        ...
        if linearizar_dif:
            ...
            fig_taxa.add_trace(go.Scatter(x=log_c, y=log_v, mode='markers', name='Pontos', marker=dict(color='cyan', size=10)))
            fig_taxa.update_layout(xaxis_title=f"ln({nome_alvo})", yaxis_title="ln(v)")
            
            # MATEMÁTICA DA TENDÊNCIA (RETA)
            if mostrar_tendencia and len(c_vals) > 1:
                z = np.polyfit(log_c, log_v, 1)
                p = np.poly1d(z)
                fig_taxa.add_trace(go.Scatter(x=log_c, y=p(log_c), mode='lines', name=f'Tendência (Ordem m={z[0]:.2f})', line=dict(color='yellow', dash='dash', width=2)))
