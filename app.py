...
st.sidebar.title("Configurações")
modelo = st.sidebar.radio("Tipo de Reação", ["A → Produto", "A + B → Produto"], key="main_model_choice")
k_sid = st.sidebar.slider("Constante k", 0.01, 2.0, 0.45, key="main_k_slider")
ordem_a_sid = st.sidebar.slider("Ordem em A", 0, 2, 1, key="main_order_a_slider")
a0_sid = st.sidebar.slider("[A]₀ Inicial", 0.1, 5.0, 2.0, key="main_a0_slider")
t_max = st.sidebar.slider("Tempo Total", 10, 100, 50, key="main_t_max_slider")

# Resolução Cinética Principal
t = np.linspace(0, t_max, 1000)
if modelo == "A + B → Produto":
    b0_sid = st.sidebar.slider("[B]₀ Inicial", 0.1, 5.0, 2.0, key="main_b0_slider")
    ordem_b_sid = st.sidebar.slider("Ordem em B", 0, 2, 2, key="main_order_b_slider")
    sol = odeint(modelo_cinetico, [a0_sid, b0_sid, 0], t, args=(k_sid, ordem_a_sid, ordem_b_sid, modelo))
    conc_a, conc_b, conc_p = sol[:, 0], sol[:, 1], sol[:, 2]
else:
    sol = odeint(modelo_cinetico, [a0_sid, 0], t, args=(k_sid, ordem_a_sid, 0, modelo))
...
st.sidebar.divider()
st.sidebar.subheader("👁️ Exibir no gráfico?")
mostrar_a = st.sidebar.checkbox("Reagente [A]", value=True, key="view_a")
mostrar_b = st.sidebar.checkbox("Reagente [B]", value=True, key="view_b") if modelo == "A + B → Produto" else False
mostrar_p = st.sidebar.checkbox("Produto", value=True, key="view_p")

# --- Ferramentas ---
st.sidebar.divider()
st.sidebar.subheader("🧮 Ferramentas")
modo_calc = st.sidebar.selectbox("O que calcular?", ["Nenhum", "Velocidade Média", "Velocidade Instantânea"], key="tool_select")

reagente_alvo = "A"
if modo_calc != "Nenhum" and modelo == "A + B → Produto":
    reagente_alvo = st.sidebar.radio("Analisar qual?", ["A", "B"], key="tool_reagent_choice")
...
if modo_calc == "Velocidade Média":
    with col2:
        st.subheader(f"VM de {nome_alvo}")
        t1 = st.number_input("t1", 0.0, float(t_max), 5.0, key="inp_vm_t1")
        t2 = st.number_input("t2", 0.0, float(t_max), 15.0, key="inp_vm_t2")
        c1, c2 = np.interp(t1, t, conc_alvo), np.interp(t2, t, conc_alvo)
        if st.button("Revelar VM", key="btn_reveal_vm"):
            st.success(f"vm = {abs(c2-c1)/(t2-t1):.4f} M/s")
    ...
    with col2:
        st.subheader(f"VI de {nome_alvo}")
        ti = st.slider("Instante (t)", 0.0, float(t_max), float(t_max/2), key="inp_vi_ti")
        ci = np.interp(ti, t, conc_alvo)
        if st.button("Revelar VI", key="btn_reveal_vi"):
            vi = k_sid * (np.interp(ti, t, conc_a)**ordem_a_sid) * (np.interp(ti, t, conc_b)**(ordem_b_sid if modelo == "A + B → Produto" else 0) if modelo == "A + B → Produto" else 1)
            st.success(f"v = {vi:.4f} M/s")

with col1:
    fig_main.update_layout(xaxis_title="Tempo (s)", yaxis_title="Molaridade (M)", template="plotly_dark")
    st.plotly_chart(fig_main, use_container_width=True, key="plot_main_chart")
...
with c_l1:
    ...
    fig_lin = go.Figure(go.Scatter(x=t, y=y_lin, line=dict(color='orange', width=2)))
    fig_lin.update_layout(height=300, template="plotly_dark", yaxis_title=lab_lin)
    st.plotly_chart(fig_lin, use_container_width=True, key="plot_linear_chart")
...
with h1: n_comp = st.number_input("Ordem", 0.0, 3.0, 1.0, step=0.5, key="hist_n")
with h2: c_comp = st.number_input("[A]₀", 0.1, 10.0, 2.0, key="hist_c")
with h3: k_comp = st.number_input("k", 0.01, 5.0, 0.45, key="hist_k")

b_g, b_r = st.columns(2)
with b_g:
    if st.button("🚀 Gravar Curva", key="hist_btn_save"):
...
with b_r:
    if st.button("🗑️ Resetar Histórico", key="hist_btn_reset"):
        st.session_state.historico = []
        st.rerun()

fig_hist = go.Figure()
if not st.session_state.historico:
    fig_hist.add_annotation(text="Adicione curvas para comparar", showarrow=False)
else:
    for c in st.session_state.historico:
        fig_hist.add_trace(go.Scatter(x=c['t'], y=c['y'], name=c['label'], mode='lines'))

fig_hist.update_layout(
    xaxis=dict(title="Tempo (s)", range=[0, 15]), # Zoom nos primeiros 15s
    yaxis=dict(title="Conc. [A]", range=[0, 6]),
    height=450, template="plotly_dark"
)
st.plotly_chart(fig_hist, use_container_width=True, key="plot_history_chart")
